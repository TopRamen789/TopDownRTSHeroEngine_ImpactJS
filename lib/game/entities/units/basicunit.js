...
.defines(function(){
	/*
		This doesn't perform very well.
		If you take a bunch of entities and then spam movement on them, the FPS will drop.
		I'd guess the performance is taking a hit due to all of the calculations I'm doing
		in the method.
	*/
	...
	BasicUnit = ig.Entity.extend({
		...
		update: function() {
			...
			if(this.goal != null) {
				...
				if(Math.floor(this.pos.x/ig.game.collisionMap.tilesize) === this.goal.x && 
					Math.floor(this.pos.y/ig.game.collisionMap.tilesize) === this.goal.y)
					this.goal = null;
			}
			this.travelPath();
			this.parent();
