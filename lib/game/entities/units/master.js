...
/* global ig, EntityItem, EntityFireball, EntityMaster, EntityBasicSwing, EntityStructure, EntityBlob, EntitySelector, HUD */
ig.module(
	...
	EntityMaster = ig.Entity.extend({
		...
		update: function() {
			...
			if(ig.input.pressed('shoot')) {
				...
				let position = {
					x: this.currentAnim.angle == 1.5708 || this.currentAnim.angle == -1.5708 ? (100 * positionChanger) : 0,
					y: this.currentAnim.angle == 3.14159 || this.currentAnim.angle == 0 ? (100 * positionChanger) : 0
				};
		...
		debug: function() {
			ig.show('X Size  ', this.selector.size.x.toFixed(2));
			ig.show('Y Size  ', this.selector.size.y.toFixed(2));
			ig.show('X Position  ', this.selector.pos.x.toFixed(2));
			ig.show('Y Position  ', this.selector.pos.y.toFixed(2));
			// ig.show('Selected Entity Count ', this.selector.currentSelection.length);
			// ig.show('Selector Colliding ', this.selector.isColliding);
		}
	});
});
